...
__global__ void spfa_local_1d_dilated_cuda_forward_kernel(
  ...
  int lowInd = max((int)(blockIdx.x - (local_size_each_dir * (dil_factor + 1))), (int)(blockIdx.x % (dil_factor + 1)));
  int upInd = min((int)(blockIdx.x + (local_size_each_dir * (dil_factor + 1))), (int)(Q.size(0) - 1));

  // Calculate the total number of non-masked elements in the row of the mask.
  int totNonMask = ((int)((upInd - lowInd) / (dil_factor + 1))) + 1;
...
torch::Tensor spfa_local_1d_dilated_cuda_forward(
    torch::Tensor Q,
    torch::Tensor K,
    torch::Tensor V,
    torch::Tensor m,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(Q.type(), "spfa_local_forward_cuda", ([&] {
    spfa_local_1d_dilated_cuda_forward_kernel<scalar_t><<<blocks, threads, maxSMemPB>>>(
